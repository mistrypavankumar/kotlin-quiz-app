...
class GameWonFragment : Fragment() {
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        if (null == getShareIntent()?.resolveActivity(requireActivity().packageManager)) {
            // hide the share menu item if it don't resolve
            menu.findItem(R.id.share)?.isVisible = false

        }
    }

    // Creating function which share intent data
...
//        shareIntent.setType("text/plain").putExtra(Intent.EXTRA_TEXT, getString(R.string.share_success_text, args.numCorrect, args.numQuestions))
        ...
        return activity?.let {
            ShareCompat.IntentBuilder.from(it)
                ...
                .setType("text/plain")   // we forget to mention then it hides the share menu as we declare in onCreateOptionMenu function
                .intent
        }
    }

    // function for actual sharing
